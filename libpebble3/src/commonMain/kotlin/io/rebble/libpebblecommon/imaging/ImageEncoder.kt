...
object ImageEncoder {
    ...
    fun encode(argb: IntArray, width: Int, height: Int): EncodedImage {
        ...
        val palette = colors.map(::gcolor8).let { if (hasTransparency) it + TRANSPARENT else it }
    ...
    // Median cut over the screen colours the opaque pixels land on. Returns up to [maxColors]
    // distinct screen colour indices. Box choice and split point are weighted by pixel count, so a
    // large flat region doesn't lose a palette slot to a handful of stray pixels.
