 ...
 * so choosing a palette against the nominal values picks codes the screen never shows and washes
 * every image out. Matching against these instead cuts the error against the source image by
 * roughly four times.
 *
 * Measured on a Pebble Time 2 in sunlight, indexed by the low six bits of the GColor8 code
 * (0xC0..0xFF), packed 0xRRGGBB.
 */
private val MEASURED = intArrayOf(
...
// The panel's own black and white. In sunlight the screen spans about a fifth of sRGB's range, and
// a viewer adapts to that, so the palette is chosen against colours normalised to those endpoints:
// what should steer the choice is the shape of the correction, not the dimming.
private const val PANEL_BLACK = 0
private const val PANEL_WHITE = 63

private fun channel(packed: Int, shift: Int) = (packed shr shift) and 0xFF

private fun normalise(shift: Int): IntArray {
    val lo = channel(MEASURED[PANEL_BLACK], shift)
    val hi = channel(MEASURED[PANEL_WHITE], shift)
    return IntArray(MEASURED.size) {
        (((channel(MEASURED[it], shift) - lo) * 255) / (hi - lo)).coerceIn(0, 255)
    }
}

/** Screen colours normalised to the panel's black and white, one array per channel. */
internal val SCREEN_R = normalise(16)
internal val SCREEN_G = normalise(8)
internal val SCREEN_B = normalise(0)

/** Index of the screen colour closest to the given sRGB value. */
internal fun nearestScreenColour(r: Int, g: Int, b: Int): Int {
    var best = 0
