...
fun rememberSettingsItemsState(navBarNav: NavBarNav, topBarParams: TopBarParams): SettingsItemsState? {
    ...
    val launchSaveBackup = rememberSaveDocumentLauncher("application/octet-stream") { success ->
        ...
        scope.launch {
            if (success) {
                topBarParams.showSnackbar("Backup saved")
            } else {
                topBarParams.showSnackbar("Backup was not saved")
            }
        }
    }
    ...
    val launchRestoreFilePicker = rememberOpenDocumentLauncher { result ->
        result?.firstOrNull()?.let { file ->
            ...
            if (!file.fileName.endsWith(".pebblebak")) {
                scope.launch {
                    topBarParams.showSnackbar("Invalid file: expected a .pebblebak file (got '${file.fileName}')")
                }
            ...
            pendingRestoreFile?.let { file ->
                scope.launch {
                    try {
                        ...
                        topBarParams.showSnackbar("Error restoring backup: ${e.message}")
                    }
                }
            ...
            listOf(
                ...
                basicSettingsActionItem(
                    ...
                    action = {
                        scope.launch {
                            try {
                                topBarParams.showSnackbar("Creating backup...")
                                ...
                                topBarParams.showSnackbar("Error creating backup: ${e.message}")
                            }
                        }
                    },
                ),
